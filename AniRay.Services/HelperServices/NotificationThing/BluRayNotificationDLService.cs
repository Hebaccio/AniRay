...
{
    public class BluRayNotificationDLService
    {
        private readonly ILogger<BluRayNotificationDLService> _logger;
        private readonly IServiceScopeFactory _scopeFactory;
        private readonly IMessageProducer _producer;
        private readonly string _connectionString;

        public BluRayNotificationDLService(
            ILogger<BluRayNotificationDLService> logger,
        ...
        public async Task RunNotificationJob(string queueName)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogInformation($"Job already running for Dead Letter Service. Exiting.");
                    await transaction.RollbackAsync();
                    return;
                }

                _logger.LogInformation($"Lock acquired for Dead Letter Service");

                await ProcessBatches(queueName);

                _logger.LogInformation($"Committing transaction for Dead Letter Service");
                await transaction.CommitAsync();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error in BluRayNotificationJob for Dead Letter Service");
                _logger.LogInformation($"Rolling back transaction for BluRay Dead Letter Service");
                await transaction.RollbackAsync();
                throw;
            }
            finally
            {
                _logger.LogInformation($"Lock released for BluRay Dead Letter Service");
        ...
        private async Task<bool> AcquireLock(SqlConnection connection, SqlTransaction transaction)
        {
            ...
            lockCommand.Parameters.AddWithValue("@ResourceName", $"BluRayNotificationDLSJob");
        ...
        private async Task ProcessBatches(string queueName)
        {
            ...
            if (FailuresCheck(consecutiveNormalFailures, maxNormalFailures, consecutiveRabbitMqFailures, maxRabbitMqFailures))
            {
                ...
                _logger.LogInformation("All batches processed for Dead Letter Service. Job complete.");
        }
        private async Task<List<UserBluRayNotifications>> GetNextBatch(AniRayDbContext db, int batchSize)
        {
            return await db.UserBluRayNotifications
                .Include(n => n.User)
                .Include(n => n.BluRay)
                .Where(n => !n.EmailQueued && n.FailureCountBeforeQueueing >= 10)
        ...
        {
            _logger.LogInformation("No more users to notify for Dead Letter Service. Job finished.");
        }
