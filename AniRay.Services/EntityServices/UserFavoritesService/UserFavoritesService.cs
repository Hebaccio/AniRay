...
namespace AniRay.Services.EntityServices.UserFavoritesService
{
    public class UserFavoritesService :
        BaseCRUDService<UserFavoritesMU, UserFavoritesME, UserFavoritesSOU, UserFavoritesSOE, UserFavorites, UserFavoritesIRU, UserFavoritesIRE, UserFavoritesURU, UserFavoritesURE>,
        IUserFavoritesService
    {
        ...
        public override IQueryable<UserFavorites> AddGetPagedFiltersForUsers(UserFavoritesSOU search, IQueryable<UserFavorites> query)
        {
        ...
        public override IQueryable<UserFavorites> AddGetPagedFiltersForEmployees(UserFavoritesSOE search, IQueryable<UserFavorites> query)
        {
        ...
        public override async Task<ServiceResult<bool>> BeforeInsertForUsers(UserFavoritesIRU request, UserFavorites entity, CancellationToken cancellationToken)
        {
            var validationCheck = await ValidateForeignKeysForInsert(request, cancellationToken);
            if (!validationCheck.Success)
                return ServiceResult<bool>.Fail(validationCheck.Message);

            var movie = validationCheck.Data;
        ...
        private async Task<ServiceResult<Movie>> ValidateForeignKeysForInsert(UserFavoritesIRU request, CancellationToken cancellationToken)
        {
            bool userExists = await Context.Set<User>()
                .AnyAsync(x => x.Id == _currentUser.UserId, cancellationToken);

            if (!userExists)
                return ServiceResult<Movie>.Fail("User does not exist.");

            var movie = await Context.Set<Movie>()
                .FirstOrDefaultAsync(x => x.Id == request.MovieId, cancellationToken);

            if (movie == null)
                return ServiceResult<Movie>.Fail("Movie does not exist.");
            ...
            if (alreadyExists)
                return ServiceResult<Movie>.Fail("This movie is already in the user's favorites");

            return ServiceResult<Movie>.Ok(movie);
        }
        #endregion
        ...
        public override async Task<UserFavorites?> EntityGetTriggerForUpdate(int? id, UserFavoritesURU? request, CancellationToken cancellationToken)
        {
            var entity = await Context.Set<UserFavorites>().FirstOrDefaultAsync(x => x.UserId == _currentUser.UserId, cancellationToken);
            if(entity == null)
            {
        ...
        public override async Task<ServiceResult<bool>> BeforeUpdateForUsers(UserFavoritesURU request, UserFavorites entity, CancellationToken cancellationToken)
        {
        ...
        public async Task<ActionResult<UserFavoritesMU>> RemoveMovieFromFavorites(int? id, CancellationToken cancellationToken)
        {
            ...
            UserFavorites entity = await Context.UserFavorites.
                Where(uf => uf.UserId == _currentUser.UserId && uf.MovieId == id)
                .Include(uf => uf.Movie).FirstOrDefaultAsync(cancellationToken);
            if (entity == null)
                return new NotFoundResult();

            entity.Movie.Favorites -= 1;
